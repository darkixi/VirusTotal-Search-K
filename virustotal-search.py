...
VIRUSTOTAL_API2_KEY = 'APIKEY-HERE'
HTTP_PROXY = ''
HTTPS_PROXY = ''
...
class CSVLogger():
    def __init__(self, prefix, headers):
        ...
        with open(self.filename, 'wb') as CSVLoggerOutput:
            writer = csv.writer(CSVLoggerOutput)
            writer.writerow([header for header in headers])

    def PrintAndLog(self, parameters):
        with open(self.filename, 'ab') as CSVLog:
            writer = csv.writer(CSVLog)
            writer.writerow([parameters[0], parameters[1], parameters[2], parameters[3], parameters[4], parameters[5], parameters[6]])
...
def LogResult(searchTerm, comment, oResult, issuedRequest, withComment):
    ...
    if oResult['response_code'] == 1:
        ...
        oLogger.PrintAndLog(parameters)
    else:
        formats = ('%s', '%d', '%d', '%s')
...
def VirusTotalSearch(filename, options):
    ...
    headers = ('Search Term', 'Requested', 'Response', 'Scan Date', 'Detections', 'Total', 'Permalink')
    if options.comment:
